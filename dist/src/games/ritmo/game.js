...
const CONFIG = {
    ...
    offsetBeats: 0.02, // Calibrado a 0.02 (adelanta las notas visuales ~86ms para un ajuste perfecto al oído del profe)
};
...
// Audio de Fondo Real (Cloudinary - Adding the Sun a 90 BPM)
const bgMusic = new Audio('https://res.cloudinary.com/dl3t6vykm/video/upload/v1779076117/Adding_the_Sun_t9lzln.mp3');
...
// ⏱️ RELOJ MAESTRO DE SINCRONIZACIÓN RÍTMICA
// Utilizar el tiempo transcurrido del elemento de audio HTML5 para evitar drift o lag rítmico.
function getCurrentBeat() {
    if (!isPlaying) return 0;
    
    // Si la música está sonando y reporta tiempo real
    if (bgMusic.currentTime > 0) {
        return (bgMusic.currentTime * (CONFIG.bpm / 60)) - CONFIG.offsetBeats;
    }
    
    // Fallback: si el audio se está precargando o amortiguando
    const elapsed = (performance.now() - startTime) / 1000;
    return (elapsed * (CONFIG.bpm / 60)) - CONFIG.offsetBeats;
}
...
function startGame() {
    ...
    // Iniciar reproducción del audio de fondo
    bgMusic.currentTime = 0;
    bgMusic.play().catch(error => {
        console.warn("No se pudo iniciar el audio automáticamente:", error);
    });
...
function gameLoop(currentTime) {
    ...
    // El juego termina cuando la música llega a su fin o la canción termina (aprox 160 beats)
    if (bgMusic.ended || currentBeat > 180) {
        endGame();
        return;
    }
...
function handleSubliminalPulse(currentBeat) {
    ...
    // Agregamos mayor precisión y suavidad al pulso subliminal
    if (frac < 0.15) {
...
function checkHit(key) {
    ...
    // Tolerancia rítmica de 0.35 beats
...
// Al finalizar el audio de forma natural, terminar el juego
